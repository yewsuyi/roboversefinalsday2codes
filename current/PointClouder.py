...
from customtopdown import local_to_ned_global, depth_to_xy_map_downward
...
class GlobalMapper:
    ...
    def update_frame(self, depth_img, state, scanmapper, receiver, north, east, down, yaw_deg, yaw_rad):
        ...
        obs_Xcoords, obs_Zcoords = depth_to_xy_map_downward(
            depth_img,
...
async def collect_and_map_frame(receiver, mapper:GlobalMapper, state, scanmapper, manual_override=False, parser=None, use_uwb_mode=True, uwb_tag=0):

    depth_img = receiver.get_depth_frame()
    if depth_img is None:
        print("No depth frame received, skipping this cycle.")
        return # no depth data received
...
async def depthcam_pointcloud_task(
        ...
        loopdelay: float = 1.0,
        parser=None, use_uwb_mode=True,uwb_tag=0
):
    """
    ...
    try:
        while True:
            ...
            data = await collect_and_map_frame(receiver, mapper, state, scanmapper,  parser, use_uwb_mode, uwb_tag=0)
            if not data: print("--- no depthcam data ---")
            else:
                # leftspace, midspace, rightspace, mapupdated = data
                pass
